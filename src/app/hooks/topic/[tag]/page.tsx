...
export async function generateMetadata(props: Props) {
  ...
  const title = topicTitle("hook", displayName);
  const description = topicDescription("hook", displayName, tagItems.length);
  ...
  return {
    ...
    keywords: [
      tag,
      `best ${tag} claude code hooks`,
      `${tag} automation`,
      `claude code hooks ${tag}`,
      "claude code",
      "hooks",
    ],
    openGraph: {
    ...
    <div className="container py-8">
      ...
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-2">
          Best Claude Code Hooks for {displayName}
        </h1>
      ...
      <RelatedUseCases tag={tag} crossPath="/hooks" />
